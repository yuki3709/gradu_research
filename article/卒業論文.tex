...
\centerline{\tt scanf("\%lf", \&num)}から{\tt num}が入力変数であることを判定し，入力のデータ構造\\
\centerline{\tt double num}が生成される。また，変数{\tt num}は{\tt double}型なので，変数{\tt num}の判別式\\
{\tt
\centerline{num > 0}
\centerline{num == 0}
\centerline{num < 0}
...
\centerline{\tt scanf("\%lf", \&res)}から{\tt res}が入力変数であることを判定し，入力データの構造\\
\centerline{\tt int res}が生成される。また，変数{\tt res}は{\tt int}型なので，変数{\tt res}の判別式\\
{\tt
\centerline{res > 0}
\centerline{res == 0}
\centerline{res < 0}
...
\centerline{\tt res == 1}とその否定\\
\centerline{\tt !(res == 1)}がテストケース評価基準として生成される。
上記プログラムを入力としてシステムを実行した結果を以下に示す。
...
\centerline{\tt scanf("\%lf", \&num)}から{\tt num}が入力変数であることを判定し，入力データの構造\\
\centerline{\tt int num}が生成される。また，変数{\tt num}は{\tt int}型なので，変数{\tt num}の判別式\\
{\tt
\centerline{num x > 0}
\centerline{num == 0}
\centerline{num < 0}
}がテストケース評価基準として生成される。また，条件判断文{\tt if}から条件式\\
\centerline{\tt even(num) == 1}とその否定\\
\centerline{\tt !(even(num) == 1)}がテストケース評価基準として生成される。さらに，条件式に関数{\tt even()}が使用されているので，関数{\tt even()}の定義\\
{\tt
\centerline {int even(int num)　　　}\\
\centerline {\{　　　　　　　　　　　　}\\
\centerline {　return num \% 2;}\\
\centerline {\}　　　　　　　　　　　　}\\
}
が生成される。
上記プログラムを入力としてシステムを実行した結果を以下に示す。
...
\centerline{\tt　mod2 == 1}とその否定\\
\centerline{\tt !(mod2 == 1)}がテストケース評価基準として生成される。さらに，条件式に入力のない変数{\tt mod2}が使用されていて，{\tt mod2}に対して関数{\tt even()}を呼び出し，戻り値を代入しているので，関数{\tt even()}の定義\\
{\tt
\centerline {int even(int num)　　　}\\
\centerline {\{　　　　　　　　　　　　}\\
\centerline {　return num \% 2;}\\
